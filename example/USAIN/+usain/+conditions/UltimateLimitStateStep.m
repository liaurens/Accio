classdef UltimateLimitStateStep < usain.conditions.BaseConditionStep

    properties
        DESCRIPTION = "Ultimate limit state"
    end
    ...
    methods
        ...
        function identifiers = get_warning_identifiers(~)
            identifiers = [ ...
                "USAIN:UltimateLimitState:NotConverged", ...
                "USAIN:UltimateLimitState:NonUniformFailureMode"];
            % These warnings need to be suppressed because it is only relevant during the design check (and contaminates
            % the console during design optimization)
        end

        function Condition = get_conditions(~)
            Condition = UsainUtils.UltimateLimitState();
        end
